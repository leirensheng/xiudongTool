...
export default {
  ...
  computed: {
    isShowRecover() {
      ...
      let usernames = Object.keys(pidInfo).filter(one => one.includes('npm run start')).map(one => one.replace('npm run start ', ''));
      let cmds = this.tableData.filter(one => !one.status && usernames.includes(one.username)).map(one => one.cmd);
